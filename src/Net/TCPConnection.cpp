    ...
    u16 port)
{
    ...
    auto const host_buffer
        = TRY(StringBuffer::create_fill(host, "\0"sv));
    auto port_buffer = TRY(StringBuffer::create_fill(port, "\0"sv));

    struct addrinfo* res = nullptr;
    if (getaddrinfo(host_buffer.data(), port_buffer.data(), &hints,
            &res)
        < 0)
...
ErrorOr<void> TCPConnection::flush_write() const
{
    ...
    while (total_bytes_written < write_buffer.size()) {
        auto view = write_buffer.view().shrink_from_start(
            total_bytes_written);
        auto bytes_written
            = send(socket, view.data, view.size, MSG_NOSIGNAL);
        if (bytes_written < 0)
            return Error::from_errno();
...
ErrorOr<StringBuffer> TCPConnection::printable_address() const
{
    ...
    c_string res = inet_ntop(address.ss_family,
        get_in_addr((struct sockaddr*)&address), buf, sizeof(buf));
    if (res == nullptr)
        return Error::from_errno();

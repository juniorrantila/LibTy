    ...
    u16 port)
{
    ...
    auto socket = TRY(Core::System::socket(res->ai_family,
        res->ai_socktype, res->ai_protocol));
    TRY(Core::System::connect(socket, res->ai_addr,
        res->ai_addrlen));
...
ErrorOr<StringBuffer> TCPConnection::read() const
{
    ...
    while (true) {
        ...
        bytes_read
            = TRY(Core::System::recv(socket, buffer, buffer_size));
        buffer[bytes_read] = '\0';
...
ErrorOr<void> TCPConnection::flush_write() const
{
    ...
    while (bytes_written < write_buffer.size()) {
        ...
        bytes_written
            += TRY(Core::System::send(socket, view, MSG_NOSIGNAL));
    }
    write_buffer.clear();

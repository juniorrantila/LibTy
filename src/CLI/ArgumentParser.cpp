...
TEST_CASE(basic_parsing_works)
{
    ...
    auto argument_parser = ArgumentParser();

    c_string output = nullptr;
    SHOULD(argument_parser.add_option("--output"sv, "-o"sv,
        "file"sv, "the foo to bar"sv, [&output](auto argument) {
            output = argument;
        }));
    bool some_long_flag = false;
    SHOULD(argument_parser.add_flag("--some-long-flag"sv, "-f"sv,
        "foo"sv, [&some_long_flag] {
            some_long_flag = true;
        }));

    SHOULD(argument_parser.run(argc, argv));

    EXPECT("some file"sv == StringView::from_c_string(output));
    EXPECT(some_long_flag == true);
